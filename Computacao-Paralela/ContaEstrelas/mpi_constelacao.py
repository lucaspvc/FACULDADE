...
def contar_estrelas_parte(imagem_parte, limiar=200, margem_exclusao=False):
    ...
    Parâmetros:
    - imagem_parte: Parte da imagem em formato binário.
    - limiar: Valor de pixel para considerar como branco.
    - margem_exclusao: Ignorar estrelas conectadas à margem superior ou esquerda se True.
    
    Retorna:
    - Número de estrelas na parte da imagem.
    """
...
def dividir_imagem(imagem, num_processos):
    ...
    Parâmetros:
    - imagem: Imagem a ser dividida.
    - num_processos: Número de processos para dividir a imagem.
    
    Retorna:
    - Lista de partes da imagem.
    """
    ...
    for i in range(num_processos):
        if i == 0:
            # Processo 0 pega a primeira parte normalmente
...
def main():
    ...
    for caminho_imagem in arquivos_imagens:
        ...
        # Distribuir as partes da imagem para todos os processos
        parte_local = comm.scatter(partes_imagem, root=0)
        
        # Cada processo conta as estrelas em sua parte
        if rank == 0:
            # Processo 0 conta estrelas normalmente
            num_estrelas_local = contar_estrelas_parte(parte_local, limiar=limiar_branco)
        else:
            # Outros processos contam com margem de exclusão
        ...
        if rank == 0:
            # Somar os resultados de todos os processos
