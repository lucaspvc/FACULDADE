...
with(Dataset, Barplot(Aprendeu,
  xlab = "Forma que o trabalhador de TI aprendeu a programar", ylab = "Frequencia",
  label.bars = TRUE
))
with(Dataset, Barplot(Aprendeu,
  xlab = "Forma que o trabalhador de TI aprendeu a programar", ylab = "Frequencia",
...
with(Dataset, piechart(Atuacao,
  xlab = "Atuacao do entusiasta de TI", ylab = "", main = "Atuacao", col = rainbow_hcl(3),
  scale = "percent"
))
library(abind, pos = 17)
library(e1071, pos = 18)
numSummary(Dataset[, "Idade", drop = FALSE], statistics = c("mean", "sd", "quantiles", "CV"), quantiles = c(
  0, .25, .5, .75,
  1
))